...
class SuccessAddMoreThanOneProductUntilCheckout:
    def __init__(self, types):
        self.__name_product = None
        self.__total_item = None
        self.__types_data = 0
        self.__status_checkout_product = "VALID"
        self.__driver = None
        self.__flow_add_product_in_bucket = None
        if types == 1:
            self.__flow_add_product_in_bucket = SuccessAddMoreThanOneProductInBucket()
            self.__driver = self.__flow_add_product_in_bucket.get_driver()
        # elif types == 2:
        #     # self.__flow_add_product_in_bucket = SuccessAddMoreThanOneProductMoreThanOneItemsInBuckets()
        #     self.__driver = self.__flow_add_product_in_bucket.get_driver()
        #     self.__types_data = types

    def main_one_items(self, list_search_data):
        self.__flow_add_product_in_bucket.main_without_close_web(list_search_data)
        GlobalFunction.delay(5)

        self.__name_product = self.__flow_add_product_in_bucket.get_list_name_product()
    ...
    def validate_checkout(self):
        ...
        self.__name_product.reverse()
        for index in range(len(self.__name_product)):
            xpath_checkout_name_product = "//*[@id='__skipper']/div[1]/div/div/div[1]/div[2]/div/div[2]/div/div/div/div["+str(index+1)+"]/div[2]/div/div/p[1]"
            element_checkout_name_product = self.__driver.find_element(By.XPATH, xpath_checkout_name_product)
            checkout_name_product = element_checkout_name_product.text
            print(self.__name_product[index])
            print(checkout_name_product)
            if self.__name_product[index] == checkout_name_product:
            ...
            if self.__types_data == 2:
                xpath_checkout_total_items = "//*[@id='__skipper']/div[1]/div/div/div[1]/div[2]/div/div[2]/div/div/div/div["+str(index+1)+"]/div[2]/div/div/p[2]/span[1]"
                element_checkout_total_items = self.__driver.find_element(By.XPATH, xpath_checkout_total_items)
